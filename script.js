...
function fmtDateKey(y,m,d){ return `${y}-${pad(m+1)}-${pad(d)}` }

// Reveal on scroll
const reveal = () => {
  const winH = window.innerHeight;
  $$('section').forEach(s => { if(s.getBoundingClientRect().top < winH - 80) s.classList.add('visible') })
}
...
function buildCalendar(){
  ...
  ptWeek.forEach(w => {
    ...
    el.className = 'dow'; el.textContent = w; calGrid.appendChild(el);
  });

  const first = new Date(view.y, view.m, 1);
  const startDay = first.getDay();
  const last = new Date(view.y, view.m + 1, 0).getDate();
  for(let i=0;i<startDay;i++){ const e=document.createElement('div'); e.className='day muted'; calGrid.appendChild(e) }
  ...
  for(let d=1; d<=last; d++){
    ...
    if(dateObj < today){ cell.classList.add('disabled'); cell.disabled = true; }
...
function selectDate(y,m,d, cell){
  ...
  $('#selectedDate').textContent = new Date(y,m,d).toLocaleDateString('pt-BR');
  $('#hiddenDate').value = key;
  renderSlots(key);
}

$('#prevMonth').addEventListener('click', ()=>{ view.m--; if(view.m<0){view.m=11; view.y--} buildCalendar(); });
$('#nextMonth').addEventListener('click', ()=>{ view.m++; if(view.m>11){view.m=0; view.y++} buildCalendar(); });
...
async function renderSlots(dateKey){
  const wrap = $('#slots'); wrap.innerHTML='Carregando horários...';
  ...
  try{
    ...
    all.forEach(time => {
      ...
      btn.type='button'; btn.className='slot'; btn.textContent=time;
...
$('#bookingForm').addEventListener('submit', async (e) => {
  ...
  if(!dateKey){ alert('Selecione uma data no calendário.'); return }
  if(!time){ alert('Selecione um horário disponível.'); return }
...
(function init(){
  buildCalendar();
  const t = new Date(); if(t.getHours()>=18) t.setDate(t.getDate()+1);
